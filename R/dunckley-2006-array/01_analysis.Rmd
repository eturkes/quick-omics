...
DE_fit <- topTable(
  fit, coef = paste0("groups", conditions[2]), number = Inf, p.value = 0.05, adjust.method = "holm"
)
...
```{r, gsva}
gene_sets <- getGmt(file.path(assets_dir, "..", "gene-sets", paste0("c5.bp.v7.0.symbols", ".gmt")))

rds <- file.path(
  assets_dir, "cache", paste0("0", analysis_no), paste0("c5.bp.v7.0.symbols", ".rds")
)
if (file.exists(rds)) {
  gsva <- readRDS(rds)
} else {
...
DE_cutoff <- gsva[rownames(DE_fit), ]
...
heatmap(
  DE_cutoff, ColSideColors = sample_color_map,
  xlab = data_name, margins = c(2, 50),
  col = color, ylab = "c5.bp.v7.0.symbols",
  labRow = toupper(gsub("^GO_|^REACTOME_", "", rownames(DE_cutoff))), labCol = " ",
