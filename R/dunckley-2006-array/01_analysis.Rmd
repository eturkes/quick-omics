...
packages <- c("conflicted", "maEndToEnd", "hgu133plus2.db")
...
gg_df <- data.frame(
  PC1 = PCA$x[ , 1],
  PC2 = PCA$x[ , 2],
  Disease = pData(data)$Characteristics..clinical.information.,
...
disease_names <- ifelse(
  str_detect(pData(data)$Characteristics..clinical.information., "histopathologically"),
  "Normal",
  "NFT"
)
heatmap_anno <- data.frame(Disease = disease_names)
row.names(heatmap_anno) <- row.names(pData(data))
dists <- as.matrix(dist(t(exprs(data)), method = "manhattan"))
rownames(dists) <- row.names(pData(data))
...
sample_threshold <- min(table(pData(data)$Characteristics..clinical.information.))
...
normal <- data[ , grep("histopathologically", pData(data)$Characteristics..clinical.information.)]
NFT <- data[ , grep("neurofibrillary", pData(data)$Characteristics..clinical.information.)]
```
...
comb_mat <- removeBatchEffect(comb_mat, batch)
